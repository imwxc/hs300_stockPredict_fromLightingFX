...
def train_lstm(train_dir,time_step=args.time_step, val=True):
    try:
        os.makedirs(args.train_graph_dir)
        os.makedirs(args.val_graph_dir)
        os.makedirs(args.train_model_dir)
        os.makedirs(args.fining_turn_model_dir)
    except:
        pass
    ...
    batch_index, val_index, train_x, train_y, val_x, val_y = get_train_data(train_dir)
...
# 其迭代次数不应过大，避免过拟合
def fining_tune_train(old_data_dir,new_data_dir,time_step=args.time_step):
    X = tf.placeholder(tf.float32, shape=[None, time_step, args.input_size])
    Y = tf.placeholder(tf.float32, shape=[None, 1, args.output_size])
    train_x, train_y = get_update_data(old_data_dir,new_data_dir)
    pred, _ = lstm_model(X)
    loss = tf.reduce_mean(tf.nn.softmax_cross_entropy_with_logits(logits=pred, labels=Y))
    train_op = tf.train.AdamOptimizer(args.lr).minimize(loss)
    saver = tf.train.Saver(tf.global_variables(), max_to_keep=15)
...
# -----------------------------测试模型------------------------------ #
# 用于测量测试集的准确率和F1
def test(test_data_dir,time_step=args.time_step,mode='Train'):
    X = tf.placeholder(tf.float32, shape=[None, time_step, args.input_size])
    test_x, test_y = get_test_data(test_data_dir)
    print("---------数据加载完毕--------")
    pred, _ = lstm_model(X)
    pre_dict = []
    ...
    with tf.Session() as sess:
        ...
        if mode=="Train":
            ...
            saver.restore(sess, args.fining_turn_model_dir + args.model_name_ft)
        print("----------模型加载完毕----------")
        if len(test_x) < 15000:
